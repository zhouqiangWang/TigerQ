package com.tiggerq.microservices.currencyconversionservice;
...
public class CurrencyConversionController {
    ...
    public CurrencyConversion retrieveCurrencyConversion(
        ...
        ResponseEntity<CurrencyConversion> responseEntity = new RestTemplate().getForEntity("http://localhost:8000/currency" +
                        "-exchange/from/{from}/to/{to}", CurrencyConversion.class,
                uriVariables);
    ...
    public CurrencyConversion retrieveCurrencyConversionFeign(
            @PathVariable String from, @PathVariable String to,
            @PathVariable BigDecimal quantity) {
        logger.info("calculateCurrencyConversionFeign called with {} to {} " +
                "with " +
                "{}", from, to, quantity);
